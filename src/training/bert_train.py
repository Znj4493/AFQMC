...
from src.data.data_loader import load_train_data, split_train_val, apply_data_augmentation
...
def save_evaluation_report(history, final_metrics, config_info, save_dir):
    """
    生成 Markdown 格式的评估报告
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        ...
        # 训练历史
        f.write("## 训练历史\n\n")
        f.write("| Epoch | Train Loss | Val Loss | Val Accuracy | Val F1-Macro |\n")
        f.write("|-------|------------|----------|--------------|---------------|\n")
        for i, (tl, vl, va, vf) in enumerate(zip(
            ...
            f.write(f"| {i+1} | {tl:.4f} | {vl:.4f} | {va:.4f} | {vf:.4f} |\n")
        f.write("\n---\n\n")
        ...
        f.write(f"| Precision (Weighted) | {final_metrics['precision_weighted']:.4f} |\n")
        f.write(f"| Recall (Weighted) | {final_metrics['recall_weighted']:.4f} |\n")
        f.write(f"| F1-Score (Weighted) | {final_metrics['f1_weighted']:.4f} |\n")
        f.write("\n---\n\n")
        ...
        if 'confusion_matrix' in final_metrics:
            ...
            f.write("## 混淆矩阵\n\n")
            f.write("|  | 预测 Label 0 | 预测 Label 1 |\n")
            f.write("|--|-------------|-------------|\n")
            f.write(f"| **真实 Label 0** | {cm[0][0]} | {cm[0][1]} |\n")
            f.write(f"| **真实 Label 1** | {cm[1][0]} | {cm[1][1]} |\n")
            f.write("\n---\n\n")
...
def main():
    ...
    # 数据增强（如果启用）
    if hasattr(config, 'USE_DATA_AUGMENTATION') and config.USE_DATA_AUGMENTATION:
        train_df = apply_data_augmentation(
            train_df,
            strategy=config.AUGMENTATION_STRATEGY,
            alpha=config.AUGMENTATION_ALPHA,
            num_aug=config.NUM_AUGMENTED_SAMPLES,
            augment_both=config.AUGMENT_BOTH_SENTENCES
        )
        print(f"数据增强后训练集: {len(train_df)} 样本")
    ...
    if config.LOSS_TYPE == 'focal':
        ...
        if config.USE_CLASS_WEIGHT:
            ...
            print(f"使用手动类别权重: label 0 = {class_weights[0]:.4f}, label 1 = {class_weights[1]:.4f}")
            print(f"权重比例: {class_weights[1]/class_weights[0]:.2f}:1")
            criterion = create_loss_function(
                loss_type='ce',
                class_weights=class_weights
            )
        else:
            criterion = create_loss_function(loss_type='ce')
    else:
    ...
    print(f"使用对抗训练: {hasattr(config, 'USE_ADVERSARIAL') and config.USE_ADVERSARIAL}")
    if hasattr(config, 'USE_ADVERSARIAL') and config.USE_ADVERSARIAL:
        print(f"  - 对抗训练Epsilon: {config.ADV_EPSILON}")
    print(f"使用数据增强: {hasattr(config, 'USE_DATA_AUGMENTATION') and config.USE_DATA_AUGMENTATION}")
    if hasattr(config, 'USE_DATA_AUGMENTATION') and config.USE_DATA_AUGMENTATION:
        print(f"  - 增强策略: {config.AUGMENTATION_STRATEGY}")
        print(f"  - 增强强度: {config.AUGMENTATION_ALPHA}")
        print(f"  - 每样本增强数: {config.NUM_AUGMENTED_SAMPLES}")
    ...
    # 生成详细的评估报告（添加混淆矩阵和分类报告）
    from sklearn.metrics import confusion_matrix, classification_report
    final_metrics['confusion_matrix'] = confusion_matrix(all_labels, all_preds)
    final_metrics['classification_report'] = classification_report(all_labels, all_preds,
                                                                    target_names=['Label 0', 'Label 1'])

    # 准备配置信息
    config_info = {
        "模型": config.MODEL_NAME,
        "最大序列长度": config.MAX_LENGTH,
        "Batch Size": config.BATCH_SIZE,
        "梯度累积步数": config.GRADIENT_ACCUMULATION_STEPS,
        "有效 Batch Size": config.BATCH_SIZE * config.GRADIENT_ACCUMULATION_STEPS,
        "学习率": config.LEARNING_RATE,
        "训练轮数": config.NUM_EPOCHS,
        "使用 AMP": config.USE_AMP and device.type == 'cuda',
        "损失函数": config.LOSS_TYPE,
        "Focal Alpha": config.FOCAL_ALPHA if config.LOSS_TYPE == 'focal' else 'N/A',
        "Focal Gamma": config.FOCAL_GAMMA if config.LOSS_TYPE == 'focal' else 'N/A',
        "使用类别权重": config.USE_CLASS_WEIGHT if config.LOSS_TYPE == 'ce' else 'N/A',
        "类别权重": str(config.CLASS_WEIGHTS) if config.LOSS_TYPE == 'ce' and config.USE_CLASS_WEIGHT else 'N/A',
        "使用对抗训练": hasattr(config, 'USE_ADVERSARIAL') and config.USE_ADVERSARIAL,
        "对抗训练Epsilon": config.ADV_EPSILON if hasattr(config, 'USE_ADVERSARIAL') and config.USE_ADVERSARIAL else 'N/A',
        "使用数据增强": hasattr(config, 'USE_DATA_AUGMENTATION') and config.USE_DATA_AUGMENTATION,
        "增强策略": config.AUGMENTATION_STRATEGY if hasattr(config, 'USE_DATA_AUGMENTATION') and config.USE_DATA_AUGMENTATION else 'N/A',
        "增强强度Alpha": config.AUGMENTATION_ALPHA if hasattr(config, 'USE_DATA_AUGMENTATION') and config.USE_DATA_AUGMENTATION else 'N/A',
        "每样本增强数": config.NUM_AUGMENTED_SAMPLES if hasattr(config, 'USE_DATA_AUGMENTATION') and config.USE_DATA_AUGMENTATION else 'N/A',
        "评估指标": config.METRIC_FOR_BEST_MODEL,
